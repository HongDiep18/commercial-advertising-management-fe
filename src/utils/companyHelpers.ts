...
export function maskIntroductionCompanyNames(text: string, companyName: string): string {
    ...
    maskedText = maskedText.replace(/「[^」*]+(責任有限公司|股份有限公司|有限公司)」/g, '「****責任有限公司」')

    maskedText = maskedText.replace(/(?<![*「])[一-龥]{2,}(責任有限公司|股份有限公司|有限公司)(?!」)/g, '****責任有限公司')
    return maskedText
}

// Map category name (as stored in company data) to category ID (for translations)
