...
export default function LanguageSelector({ variant = "desktop" }: LanguageSelectorProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        onClick={() => setIsLanguageDropdownOpen(!isLanguageDropdownOpen)}
        className="hover:bg-header-red-dark flex cursor-pointer items-center gap-1.5 rounded px-2 py-1.5 text-white transition-colors"
      >
        <Globe className="h-4 w-4" />
        <span className="hidden text-sm font-medium sm:inline">{languages[currentLanguage]}</span>
